...
function cm.filter1(c,tp)
	return c:IsCode(37564765) and c:IsPosition(POS_FACEUP_ATTACK) and c:IsAttackable() and Duel.IsExistingTarget(cm.filter2,tp,0,LOCATION_MZONE,1,nil,c)
end
...
function cm.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g1=Duel.SelectTarget(tp,cm.filter1,tp,LOCATION_MZONE,0,1,1,nil,tp)
	local ac=g1:GetFirst()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_OPPO)
	local g2=Duel.SelectTarget(tp,cm.filter2,tp,0,LOCATION_MZONE,1,1,nil,ac)
	e:SetLabelObject(ac)
end
function cm.cf(c,e)
